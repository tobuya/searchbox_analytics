class UserSearch < ApplicationRecord
  ...
  def self.log_search(ip_address, term)
    ...
    last_search = UserSearch.where(ip_address:).order(created_at: :desc).first
    if last_search.nil? || !term.start_with?(last_search.search_term.term)
      ...
      search_term = SearchTerm.find_or_create_by(term:)
      create(search_term:, ip_address:)
    else
  ...
  def broadcast_search
    Rails.logger.debug "Broadcasting to search_analytics_channel with term: #{search_term.term} and IP: #{ip_address}"
    ActionCable.server.broadcast('search_analytics_channel',
                                 { search_term: search_term.term, ip_address: })
